...
The app shows real-time weather for any location in Kenya. On load it automatically detects where you are using the browser's GPS. If GPS is denied or takes too long it falls back to IP-based geolocation. Once a location is resolved it fetches current conditions, a 7-day forecast, and an AI-generated weather summary from the WeatherAI API.

You can also search any Kenyan city by name or click one of five quick-select buttons — Nairobi, Mombasa, Kisumu, Nakuru, Eldoret. There is a toggle to switch all weather text and AI summaries between English and Swahili.
...
Called once on page load when GPS is not available. Returns the user's coordinates and city name based on their IP address. The `ai=false` flag is passed deliberately so this call does not consume any of the 200 monthly AI requests — it is only needed for coordinates, not a weather summary.

**`GET /v1/weather?lat=X&lon=Y&units=metric&days=7&lang=en`**
The main data call. Returns current conditions (temperature, feels-like, humidity, wind speed, UV index, condition), a 7-day daily forecast with high/low temps and rain probability, and an optional Gemini-powered AI summary. This call re-fires whenever the user selects a different city or toggles between English and Swahili.
...
The `vercel.json` file in the project root tells Vercel to proxy `/api/*` to WeatherAI and `/nominatim/*` to Nominatim. This is the production equivalent of the Vite dev proxy and is required — without it all API calls will fail in production.
